// packages/evershop/src/lib/util/hookable.js
...
function hookable(originalFunction, context) {
  ...
  // İsimli async proxy handler fonksiyonu
  async function hookableAsyncHandler(target, thisArg, argumentsList) {
  ...
  // İsimli sync proxy handler fonksiyonu
  function hookableSyncHandler(target, thisArg, argumentsList) {
    const beforeHookFunctions = beforeHooks.get(funcName) || [];
    const afterHookFunctions = afterHooks.get(funcName) || [];
